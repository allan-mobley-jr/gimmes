...
4. **Step 4c-lite: one batched conferral.** The independent review is NOT waived: every REJECT criterion, the edge pre-filter, the audit-language rule, the cross-threshold consistency check, and the APPROVE/REJECT decision-note heredoc templates from Step 4c apply verbatim, and you still MUST complete the review before dispatching Closer. The ONLY relaxation: replace sub-step 3's per-candidate SendMessage conferral with ONE batched exchange — a single SendMessage to Caddie covering ALL PROCEED candidates at once (they share one underlying event: this hour's settlement), with at most one follow-up for the whole batch. Per-candidate back-and-forth costs 5-15 minutes of wall-clock and forfeits the window. Sub-steps 4-6 remain PER-CANDIDATE — one deliberate decision, one decision note, and one skip log per ticker; ONLY the sub-step-3 conferral is batched. This is a real, named relaxation of a capital-discipline guardrail (#721); it is acceptable ONLY because the hourly lane is a paper-trading experiment — NEVER extend it to full cycles.
   **Conferral preload (#749):** Caddie's hourly PROCEED memos carry a `Conferral preload:` block — the five standard probe answers, written at research time. When EVERY candidate in the batch (the candidates reaching sub-step 3 — pre-filter and review-reuse deaths never confer) has a complete preload (all five lines present with real content), the preloads ARE the batched conferral: read them as Caddie's answers, log `Hourly 4c: conferral done — preload`, and proceed to sub-step 4 with NO SendMessage — the synchronous round-trip was the measured #749 sink (3.7-8.1 min against a sub-hour window), and under #739 the exchange gates nothing. Send the batched SendMessage ONLY when a preload is missing, incomplete, or boilerplate, or when you have a material question the preloads do not answer — never as a reflex — and then log `Hourly 4c: conferral done — exchange` when the final reply arrives. Boilerplate means content-free filler — a restated probe, "n/a", or a line that would read identically for a non-hourly candidate. Near-identical lines ACROSS a ladder's rungs are EXPECTED, not boilerplate: the rungs share one event, one formula, and one portfolio, and the sanctioned Signal-independence answer is a fixed truth of the price-anchored model — judge each preload against its own candidate, never against its siblings. Either way the review itself remains mandatory and sub-steps 4-6 are unchanged. This deepens this override's named paper-only relaxation under the same scoping — NEVER extend it to full cycles, where the full Step 4c SendMessage conferral mandate applies verbatim.
5. **Hourly review is mechanical (#739/#769).** The hourly lane trades the backtest-validated mechanical strategy; YOUR review must not become a judgment gate — but the distance verdict is now a MECHANICAL check (#769). In hourly cycles: (a) the `gimme_threshold` score intake does NOT apply — review EVERY hourly candidate with `recommendation = proceed` regardless of GimmeScore; (b) hourly probabilities come from the price-anchored formula `max(min(NO_mid + $0.10, 0.99), 0.70)`, so flat or near-flat probabilities across a ladder's rungs are the expected NORM, not an inconsistency — NEVER reject an hourly event for probability flatness under the cross-threshold consistency rule; (c) your MECHANICAL checks fully apply and are the real selectors — Shadow verdict (#769): a `proceed` whose memo carries `Shadow: WOULD-PASS` is a mechanical REJECT (that is Caddie drift — flag it; the CLI order gate blocks it regardless), edge after fees vs `cm_min_edge_after_fees` (computed from the stated prob), event/series concentration caps ("approve only the highest-edge thresholds that fit" remains the rung selector — and "fits" is mechanical, #640: when a candidate shares an event with open positions or resting orders, read `gimmes risk-check --event EVENT_TICKER` and compare remaining capacity against the candidate's FULL auto-size cost; if it does not fit, REJECT with portfolio over-concentration citing both numbers — entries are NEVER shrunk to fit, the backtest skips at the cap), STALE-CLOSE, FLIP-WARN, side constraint; (d) subjective REJECT reasons (thesis hole, signal dependence, timing) become ADVISORY for hourly candidates — record the concern in the decision note as `Concern (advisory): ...` and APPROVE if the mechanical checks pass; rejecting on judgment would relocate the exact uninstrumented gate #739 removed from Caddie. This extends the named paper-only relaxation in override 4 — NEVER extend it to full cycles.
...
7. **Step 4c latency instrumentation (#749).** The gap between Caddie completing research and your Closer dispatch is the hourly lane's dominant fill-killer: 5-13 minutes measured on 2026-07-21/22, roughly constant whether the batch held 1 candidate or 3 — fixed overhead, and every minute of it decays fill probability against the #743 approval-price cap. In hourly cycles, bracket the review with activity markers so the sink is measurable from the activity log. Log each with the standard `gimmes log-activity` command (`--agent caddie-master --phase info`), at the moment its event happens:
   - entering Step 4c: `Hourly 4c: review start — N candidates`
   - when the conferral resolves — on accepting complete preloads with no exchange: `Hourly 4c: conferral done — preload`; on a fallback exchange, when its FINAL reply arrives (after the follow-up, if you used one): `Hourly 4c: conferral done — exchange`
   - after ALL decision notes and skip logs are written: `Hourly 4c: decisions logged — X approved, Y rejected`
   - once, immediately before the FIRST Step 5 Closer dispatch: `Hourly 4c: dispatching Closer`
   A marker is NEVER skippable when its bracketing event occurs — these lines cost seconds, they sit on the Deadline Protocol's never-shed list, and they are the instrumentation #749 mandates before any review step may be cut. When an event never fires, its marker is simply OMITTED — that is not a shed and needs no skip log: a 4c pass where every candidate died on the edge pre-filter or review reuse sends no conferral (no marker 2), and an all-rejected exit dispatches no Closer (no marker 4). Marker 3 with its actual counts still fires on every path that enters 4c.
   Treat 5 minutes from Caddie completion to Closer dispatch as the working budget — and in hourly cycles the shed table's ~4-min-per-candidate arithmetic does NOT apply: the batched conferral makes review cost fixed, not per-candidate, so review the WHOLE batch whenever remaining time covers the budget plus dispatch margin rather than shedding rungs (a deferred hourly rung is not "eligible next cycle" — it settles this hour and is simply forfeited). You cannot skip the conferral, the decision notes, or any safety gate to meet the budget; meet it by adding NOTHING else — no market fetches beyond sub-step 2's per-candidate `market-info` read (which IS the #743 approved-price fetch — never fetch twice), no skipping the `--limit 3` reads the #660/#661 REJECT criteria require (those are part of the review, not additions), no re-reading Caddie memos already read this cycle, no prose beyond the decision-note templates.
