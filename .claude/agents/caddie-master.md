...
7. **Step 4c latency instrumentation (#749).** The gap between Caddie completing research and your Closer dispatch is the hourly lane's dominant fill-killer: 5-13 minutes measured on 2026-07-21/22, roughly constant whether the batch held 1 candidate or 3 — fixed overhead, and every minute of it decays fill probability against the #743 approval-price cap. In hourly cycles, bracket the review with activity markers so the sink is measurable from the activity log. Log each with the standard `gimmes log-activity` command (`--agent caddie-master --phase info`), at the moment it happens:
   - entering Step 4c: `Hourly 4c: review start — N candidates`
   - when the batched conferral reply arrives: `Hourly 4c: conferral done`
   - after ALL decision notes and skip logs are written: `Hourly 4c: decisions logged — X approved, Y rejected`
   - immediately before the Step 5 Closer dispatch: `Hourly 4c: dispatching Closer`
   These four markers cost seconds and are NEVER optional in hourly cycles — they are the instrumentation #749 mandates before any review step may be cut. Treat 5 minutes from Caddie completion to Closer dispatch as the working budget. You cannot skip the conferral, the decision notes, or any safety gate to meet it; meet it by adding NOTHING else — no market fetches beyond the single approved-price fetch per candidate, no re-reading research already read this cycle, no prose beyond the decision-note templates.
