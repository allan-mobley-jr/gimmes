...
Two gates from the worst single day of the hourly experiment: 2026-08-11 lost −$878 across two BTC entries, and the second of them exposed a governance breach. Both entries were `Shadow: WOULD-PASS` — the retired proximity gate's counterfactual said "too close" (one sat $1.56 from the strike against a $300 realized 30-minute move) while the formula's 0.70 floor asserted a 27pp edge — and the full record showed filled WOULD-PASS entries at 2W-4L, −$1,244: the entire in-band deficit. The second entry was also placed by Caddie Master itself, 73 seconds after the Closer's order command was denied by the permission classifier and correctly concluded with 0 trades. **Order placement is now Closer-only and a failed attempt is terminal for its candidate for the rest of the cycle (CLI-enforced, no --force bypass), and the hourly distance gate governs: a WOULD-PASS verdict blocks the entry at all three layers — Caddie recommendation, Caddie Master mechanical review, and a CLI order gate that reads the verdict from the research memo.** **No loop restart required after `gimmes update`** — the changed code paths (`gimmes order` / `gimmes log-trade`, agent prompts) are loaded fresh by agent subprocesses each cycle; the loop process itself is untouched. One behavior note: the hourly lane will now decline proximity rungs the backtest's band alone would have entered — the Shadow dataset keeps recording both verdicts, so the decision stays revisable with more data.

### Fixed
- **Closer-only order placement + terminal failed attempts** (#768, PR #770): two in-cycle CLI gates in `gimmes order`, active only when `GIMMES_CYCLE` is set so manual operator orders are untouched. The identity gate hard-rejects any in-cycle placement not carrying `--agent closer` (buy and sell) with an `order_agent_denied` RISK_BREACH audit row. The terminal gate refuses an in-cycle BUY when an `Order attempt terminal:` activity_log marker exists for the ticker+cycle (24h-bounded, #761 pattern) — armed by the Closer's `order_failed`/`order_canceled` skip (the only trace a classifier denial leaves; in the incident it existed 107 seconds before the Caddie Master's placement, so this alone would have blocked it) and by the order command's own four BUY failure exits. Terminal means terminal for every agent session including the Closer, turning closer.md's "NEVER retry in this cycle" into CLI law; a failed CLOSE stays retryable (the close_failed backstop depends on it). Lookup errors fail open with audit rows; a failed marker write leaves a durable `order_terminal_marker_failed` row (a lost marker disarms the gate — it gets the same visibility as a failed lookup); a loop session whose `GIMMES_CYCLE` env got lost warns that the gates are inert. Caddie Master gains the `NEVER run gimmes order` rule every other non-Closer agent already had, plus an explicit ban on the "the Closer's failure was procedural, so I'll save the trade" rationalization (revoking the #645 precedent where exactly that was treated as a good save); both CM dispatch templates now carry `--agent closer`; all pinned by drift guards, and the test suite is isolated from ambient `GIMMES_CYCLE` (a pytest run from an in-cycle shell previously mass-failed 58 order tests). Known limitation, by design: `--agent` is caller-declared (Closer and CM share one process env, so no stronger identity exists at placement time) — the gate catches honest misuse, which is what the incident was; a deliberate lie is prompt-forbidden and auditable, the same threat model as the #661 churn gate.

### Changed
- **The hourly distance gate governs — WOULD-PASS blocks entries** (#769, PR #771): #739's reserved-decision clause said re-enabling the gate is "a retrospective decision, NOT agent judgment" — #769 is that decision, made with the shadow dataset #739 existed to build, and applied mechanically in both directions (never override a WOULD-PASS to proceed; never PASS a WOULD-PROCEED on distance grounds). The issue's gentler option — suppress the 0.70 floor on proximity rungs — was checked against the data and rejected: the worst trade's unfloored probability (0.685) still cleared every edge gate, so only blocking works. Three layers per the #750/#768 doctrine: Caddie maps WOULD-PASS to `--recommendation pass` (the Shadow line template, verdict tokens, and comparators stay byte-identical for dataset continuity; UNAVAILABLE proceeds — fail-closed would pressure toward fabricated numbers, and UNAVAILABLE entries show no deficit); Caddie Master treats a `proceed` carrying WOULD-PASS as a mechanical REJECT (Caddie drift, not judgment); and the CLI order gate parses the verdict from the ticker's newest research memos for in-cycle hourly BUYs and hard-blocks WOULD-PASS with a `shadow_gate_blocked` RISK_BREACH row (auto-escalates via Groundskeeper — a working block is loud). The parser is deliberately liberal (a parse miss fails the gate OPEN): `Shadow:` with or without the space, malformed lines skipped rather than masking valid ones, bookkeeping rows (#676 shape) scanned past rather than shadowing a researched verdict, and the no-verdict audit row carries a `candidate_rows` discriminator so template drift is distinguishable from legitimate no-candidate traffic. The 0.70 floor, price-anchored formula, and #750 band are untouched — the floor stays a sizing backstop on the rungs where it is honest; the gate removes the rungs where it was fiction. Known caveat, recorded: n=6 filled WOULD-PASS entries, and the two winners also sat close to their strikes (+$271 forfeited by blocking; net effect on the realized record ≈ +$973) — the dataset keeps accumulating on both verdicts for a future threshold-shaped refinement.
