"""Integration tests for Kalshi API client (requires demo API credentials)."""
...
class TestKalshiClient:
    async def test_list_markets(self, demo_config) -> None:
        async with KalshiClient(demo_config) as client:
            markets, cursor = await list_markets(client, limit=5)
            assert len(markets) > 0
            assert markets[0].ticker

    async def test_get_balance(self, demo_config) -> None:
        async with KalshiClient(demo_config) as client:
            balance = await get_balance(client)
            assert isinstance(balance, float)
            assert balance >= 0

    async def test_get_orderbook(self, demo_config) -> None:
        async with KalshiClient(demo_config) as client:
